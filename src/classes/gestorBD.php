...
// Include the SDK
	...
	require_once(dirname(__FILE__).'/../config.inc.php');
        class GestorBD
        {
                ...
                */
                public function get_lastAccessTandemStatus($user,$id_course) {
               	...
               	* @return boolean
                */
    			public function register_course($courseKey, $title, $waiting_room) {
                	$result = false;
                	$sql = 'INSERT INTO course (courseKey, title, use_waiting_room, created)
                				VALUES ('.$this->escapeString($courseKey).','.$this->escapeString($title).', '.($waiting_room?1:0).', now())';
          			$result = $this->consulta($sql);
                	return $result;
                }
            	...
            	* @return unknown
                */
                public function update_course($courseKey, $title, $waiting_room=false) {
    	            $result = false;
	                $sql = 'UPDATE course SET title = '.$this->escapeString($title).','
                                . 'use_waiting_room ='.($waiting_room?1:0).' '.
        ...
        /**********************************************************************************/
        /**********************************************************************************/
        /******* W A I T I N G      R O O M    M A N A G E M E N T ************************/
        /**********************************************************************************/
        /**********************************************************************************/
        /**
         ...
         * @return boolean
         */
         private function addOrRemoveUserToWaitingRoom($id_waiting_room, 
                 $number_user_to_add_or_remove){
            $ok = false;
         ...
         * @return type
         */
        public function updateUserAndRoom($language,$idCourse){
            $result = $this->consulta("UPDATE waiting_room SET language = ".$language." WHERE id_course = ".$idCourse);
            return $result;
        }
         ...
         * @return type
         */
        public function deleteUserAndRoom($language,$idCourse){
            echo $language,$idCourse.'<br>';
            $result = $this->consulta('DELETE FROM waiting_room where id_course = '.$idCourse);
